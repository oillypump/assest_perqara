...
@task(name="fetching stg_products", log_prints=True)
...
@flow()
def cln_stg_products():
    """1. fetching from sql db stg"""
    table_source = "stg_products"
    df = fetching_data(table_source)

    """2. transform """
    df = transform_stg_products(df)
    """3. insert into db cln"""
    table_target = "t_products"
