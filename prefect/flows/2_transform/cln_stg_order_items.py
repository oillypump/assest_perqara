...
@task(name="fetching stg_order_items", log_prints=True)
...
def transform_stg_order_items(df):
...
def insert_to_db_cln(df, table_target):
    database_block = SqlAlchemyConnector.load("perqara-database")
    with database_block.get_connection(begin=False) as engine:
        df.to_sql(table_target, schema="cln", con=engine, if_exists="replace", index=False)


@flow()
def cln_stg_order_items():
    """1. fetching from sql db stg"""
    table_source = "stg_order_items"
    df = fetching_data(table_source)

    """2. transform """
    df = transform_stg_order_items(df)
    """3. insert into db cln"""
    table_target = "t_order_items"
