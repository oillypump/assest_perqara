...
@task(name="fetching stg_customers", log_prints=True)
...
def transform_stg_customers(df):
...
@flow()
def cln_stg_customers():
    """1. fetching from sql db stg"""
    table_source = "stg_customers"
    df = fetching_data(table_source)

    """2. transform """
    df = transform_stg_customers(df)
    """3. insert into db cln"""
    table_target = "t_customers"
